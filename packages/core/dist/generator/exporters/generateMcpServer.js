 ...
 */
function generateMcpServer(mcpJson, outputPath, apiBaseUrl) {
    const serverCode = `
...
`;
    fs_1.default.writeFileSync(path_1.default.resolve(outputPath, 'mcp-server.ts'), serverCode.trim());
    console.log(`✅ Generated MCP server at ${path_1.default.join(outputPath, 'mcp-server.ts')}`);
 ...
 */
function generateToolDefinitions(mcpJson, apiBaseUrl) {
    if (!mcpJson.actions || mcpJson.actions.length === 0) {
        return 'const toolDefinitions = [];';
    }
    const toolDefs = mcpJson.actions.map((action) => {
 ...
 */
function generateActionUrl(action, apiBaseUrl) {
 ...
 */
function generateToolFunctionBody(action, apiBaseUrl) {
...
//# sourceMappingURL=generateMcpServer.js.map