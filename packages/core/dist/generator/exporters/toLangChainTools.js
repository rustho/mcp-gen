...
exports.generateLangChainTools = generateLangChainTools;
 ...
 */
function generateLangChainTools(mcpJson, outputDir, apiBaseUrl) {
    // Ensure the output directory exists
    if (!fs_1.default.existsSync(outputDir)) {
        fs_1.default.mkdirSync(outputDir, { recursive: true });
    }
    const toolsOutputPath = path_1.default.join(outputDir, "langchain-tools.ts");
    const loaderOutputPath = path_1.default.join(outputDir, "langchain-toolloader.ts");
    // Generate the tools file
    const toolsFileContent = generateToolsFile(mcpJson, apiBaseUrl);
    fs_1.default.writeFileSync(toolsOutputPath, toolsFileContent);
    // Generate the tool loader file
    const loaderFileContent = generateToolLoaderFile(mcpJson);
    fs_1.default.writeFileSync(loaderOutputPath, loaderFileContent);
 ...
 */
function generateToolsFile(mcpJson, apiBaseUrl) {
    var _a;
...
export const tools: Tool[] = [\n`;
    ...
    (_a = mcpJson.actions) === null || _a === void 0 ? void 0 : _a.forEach((action) => {
      ...
      try {
        ...
        ${action.method.toUpperCase() === "GET"
          ...
          ${action.method.toUpperCase() !== "DELETE"
                ? `body: JSON.stringify(${params.filter(param => !action.path.includes(`{${param[0]}}`)).length > 0
                    ? "params"
                    : "{}"}),`
                : ""}
        });`}
 ...
 */
function generateToolLoaderFile(mcpJson) {
    var _a;
...
//# sourceMappingURL=toLangChainTools.js.map